...
// recursive method that takes a String and returns that string backwards
	public String backString(String input) {
		if (input.isEmpty()) {
			return input;
		}
		return backString(input.substring(1)) + input.charAt(0);
	}
	...
	public boolean equalString(String s1, String s2) {
		...
		if(s1.charAt(0)==s2.charAt(0)){
			...
			return equalString(s1.substring(0, s1.length()-1), s2.substring(0, s2.length()-1));
		}
		else {
			return false;
		}
	...
	// recursive method that takes a list of strings and returns the number of items in the string
		public int listSize(List<String> l) {
			ArrayList<String> lc = new ArrayList<String>(l);
			if (lc.isEmpty()) {
				return 0;
			}
			else {
